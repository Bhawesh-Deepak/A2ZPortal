jQuery('document').ready(function($){var slide=$('.slider-single');var slideTotal=slide.length-1;var slideCurrent=-1;let vid=$('.slider-single-image');let title=$('.slider-single-title');vid.onplaying=function(){title.toggleClass('disabled');}
vid.paused=function(){title.css('display','block');alert('pause');}
function slideInitial(){slide.addClass('proactivede');setTimeout(function(){slideRight();},500);}
function slideRight(){vid[0].pause();if(slideCurrent<slideTotal){slideCurrent++;}else{slideCurrent=0;}
...
function slideLeft(){vid[0].pause();if(slideCurrent>0){slideCurrent--;}else{slideCurrent=slideTotal;}
...
var left=$('.slider-left');var right=$('.slider-right');left.on('click',function(){slideLeft();});right.on('click',function(){slideRight();});slideInitial();});